...
const getBaseStatGreaterThan = (arr, minBaseStat) => arr.filter(currentValue => currentValue.baseStat > minBaseStat);
...
const getStatName = (arr, minBaseStat) => arr.filter(filterValue => filterValue.baseStat > minBaseStat).map(mapValue => mapValue.stat.name);
...
const getCharactersWithoutChildren = (arr) => arr.filter(currentValue => !Array.isArray(currentValue.children));
...
const evenOddNumericValues = (arr) => arr.filter(filterValue => Number.isInteger(filterValue)).map(mapValue => mapValue % 2 === 0 ? 'even' : 'odd');
