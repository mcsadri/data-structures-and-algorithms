...
const raisedToTheThird = (arr) => {
  const cubes = [];
  arr.forEach(currentValue => cubes.push(Math.pow(currentValue, 3)));
  return cubes;
};
...
const addOne = (arr) => {
  const mappedArr = arr.map(currentValue => currentValue + 1);
  return mappedArr;
};
...
const addQuestion = (arr) => {
  const questions = arr.map(currentValue => `${currentValue}?`);
  return questions;
};
...
const forLoopTwoToThe = (arr) => {
  // You may choose to complete this challenge using a for loop, for...in syntax, or for...of syntax.
  // I chose not to.
  const powers = arr.map(currentValue => Math.pow(2, currentValue));
  return powers;
};
...
const forEachTwoToThe = (arr) => {
  const powers = [];
  arr.forEach(currentValue => powers.push(Math.pow(2, currentValue)));
  return powers;
};
...
const charCode = (arr) => {
  const ints = arr.map(currentValue => currentValue.charCodeAt(0));
  return ints;
};
...
const extractAbilities = (arr) => {
  const names = arr.map(currentValue => currentValue.ability.name);
  return names;
};
...
const extractStats = (arr) => {
  const statTotals = arr.map(currentValue => ({
